 ...
 *      (2)、RedisManager 帮我们创建ReidsCache来作为缓存组件；RedisCache通过操作redis缓存数据的
 *      (3)、默认保存数据 k-v 都是Object:利用序列化来保存；如何保存json
 *            1、引入了redis的stater,CacheManager变为RedisCacheManager;
 *            2、默认创建的 RedisCacheManager操作redis的时候，使用的是RedisTemplate<Object,Object>
 *            3、RedisTemplate<Object,Object>是默认使用jdk的序列化机制
 *      (4)、自定义CacheManager
 */

@MapperScan("com.lannister.cache.mapper")
